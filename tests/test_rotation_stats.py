...
def test_cohort_ci_brackets_the_observed_mean():
    mean, lo, hi, n_an, _ = cohort_cosine(_cells(0.6), n_boot=500)
    assert lo < mean < hi and n_an == 4
...
def test_animal_without_bootstrap_contributes_its_point_estimate():
    ...
    mean, _, _, n_an, _ = cohort_cosine(cells, n_boot=300)
    assert n_an == 4
    assert mean == pytest.approx(np.mean([0.6, 0.6, 0.6, 0.2]), abs=1e-9)


def test_cohort_ci_narrows_with_more_animals():
    _, lo4, hi4, _, _ = cohort_cosine(_cells(0.6, n_animals=4), n_boot=1000)
    _, lo12, hi12, _, _ = cohort_cosine(_cells(0.6, n_animals=12), n_boot=1000)
