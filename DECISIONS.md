...
## 2026-09-12 — CHRONIC is ACCURACY-ONLY: the licks/trial arm is dropped from the boundary

Priya: "I don't think I trust that result. Maybe we should get rid of the licks per trial plateau and just look at when accuracy plateaus?" — after registering the 9/11 sessions moved the derived chronic boundaries in a way that read as noise, not recovery.

**What the licks arm did on 9/11.** The chronic rule was an AND of two series (see [2026-09-07 — The CHRONIC epoch: an AND of two measures](#2026-09-07--the-chronic-epoch-an-and-of-two-measures-not-a-composite-of-them)): chronic began at the LATER of the far_R hit-rate plateau and the far_R licks/trial plateau, both required. Registering PS92_0911 and PS93_0911 re-derived the boundaries to **PS92 chronic → None (regressed OUT of chronic)** and **PS93 chronic 11 → 18**. Both moves came entirely from the licks arm: PS92's hit-rate plateau held at day 11 (102% of baseline) but its licks tail stopped being flat, and PS93's licks plateau moved to day 18. PS92's 9/11 session was **314 trials** — short and low-engagement for it (usual ~380–600) — so a single off session, on the noisier of the two series, flipped an animal out of a recovery stage. The "chronic from this session onward" flat test makes the last point load-bearing, which is exactly where a noisy series does the most damage.

**Why accuracy-only, not a looser licks tolerance.** licking is ~3.5× noisier than hit rate (already reflected in the per-series SD scaling), so it will keep tripping the boundary as sessions accrue; loosening `k_drift` is a knob that needs re-tuning every few nights and also loosens the hit test (there is one shared multiplier, not a per-series one). Accuracy-only is the principled choice and matches the rationale that retired the level bars on 09-10 — "the plateau after the acute→subacute transition should be enough." It does not misclassify PS94: PS94's **hit rate never plateaus**, so it stays non-chronic on accuracy alone (the licks arm was not what protected it). And on the current cohort the derived boundaries are **unchanged from the ratified spec** — PS92 d11, PS93 d11, PS94 None, PS95 d15 — so no published epoch moved; only the future stability changes.
...
## 2026-09-12 (late) — On LICK TRIALS the reorganisation SURVIVES matching, and it is not a window artefact

This **partially qualifies the withdrawal above**, which says nothing survives training-set matching
anywhere. That is now true only of the ALL-TRIALS arms.

The `cuelick` / `ENLlick` arms (33a3417) hold the trial set fixed at lick trials and move only the
window. Matched family, gap as change from pre, Bonferroni-corrected over nine contrasts:

| window | acute | subacute | chronic |
|---|---|---|---|
| post-lick | **+0.179 [0.057, 0.272]** | **+0.112 [0.032, 0.222]** | +0.084 [−0.001, 0.181] |
| **post-cue** | **+0.158 [0.020, 0.314]** | +0.105 [−0.002, 0.174] | **+0.090 [0.010, 0.175]** |
| pre-cue | **+0.117 [0.041, 0.236]** | +0.021 [−0.038, 0.089] | +0.035 [−0.017, 0.101] |

### What the window test was for, and exactly what it shows

The post-lick arm was the only surviving reorganisation signal, and two accounts predicted it:

    CODING     the position code moved into a form the frozen readout cannot reach
    EXECUTION  the post-lick window is dominated by activity generated by performing the lick;
               the lick differs by spout position and CHANGES after the lesion, so a refit decoder
               reads the new movement pattern and the frozen one cannot

**They differ in where the effect should live.** Under EXECUTION it should be concentrated in the
window containing the movement and fall away before it — in particular it should be weak or absent
in the ENL window, which precedes both the cue and the lick.

Acutely it is present in all three (+0.179, +0.158, +0.117), with only a mild gradient toward
post-lick. **It does not require the movement window**, which is evidence against EXECUTION as the
whole story.

**THAT IS WEAKER THAN "IT IS POSITION CODING", and the earlier phrasing overstated it.** Two reasons:

1. **The three windows are not independent tests.** They are adjacent in time on the SAME trials and
   cortical activity is autocorrelated, so a signal in one leaks into its neighbours. This rules out
   a strictly movement-locked account; it does not establish an independent pre-movement one.
2. **The epoch pattern does not fit either account cleanly.** Acute appears everywhere; chronic
   survives ONLY post-cue; pre-cue is acute-only. A pure coding account predicts no such
   window-dependence at chronic.

The defensible statement is therefore: *the effect is not confined to the movement window*, not
*the effect is position coding*.

### The sharper finding: the MISS trials are what destroy it

Post-cue, matched, same window, only the trial set differs:

| trial set | acute | chronic |
|---|---|---|
| `working` (lick + miss-while-working) | +0.043 crosses | +0.074 crosses |
| **`lick` only** | **+0.158 EXCLUDES** | **+0.090 EXCLUDES** |

**On trials the animal completed, a reorganisation signal survives matching and correction. Adding
the failed trials destroys it.** That is a coherent and interesting claim, and it is the only one
today that holds in the matched family.

### What is NOT settled

**Selection.** The window test separates WINDOW from CODING; it says nothing about whether the
lick-trial POPULATION changes with the deficit — and it does. Acute far-contralateral is still gated
out of these arms below `MIN_REFIT_SHARE` (0.0–4.3% of trials against a pre-stroke 16.5%), so the
acute row is effectively over five positions. "The code reorganised on completed trials" and "the
completed trials are a different population" both remain live.

Both corrected lower bounds are thin (+0.020 acute, +0.010 chronic post-cue) and chronic rests on
three animals.

### The corroborating evidence is currently on the WRONG TRIAL SET

Crossnobis displacement, best-match destination and the confusion matrices — the measures that agree
about WHERE the code moved — are all computed on `working`. The claim now rests on `lick`. **They do
not currently converge on the same trials.**

No code change is needed: the `ARMS` loop at `epoch_grant_figures.main` wraps EVERY family, so
