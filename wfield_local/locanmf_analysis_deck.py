...
def build_analysis_deck(src: Path, out_path: Path, dates=None, animals=None, tag=None, allow_missing=0,
    ...
    _EPOCH = (
        ...
        ("epoch_14_beta_maps_*_*.png",
         "WHERE the position code lives, and where it goes -- cortical decoder maps",
         "THE ANATOMICAL ARM. Every other family in this section answers MOVED TOWARD WHAT, not"
         "MOVED WHERE: best-match destination collapses 380 features into one correlation per"
         "position pair and takes an argmax, which names a representational destination and not a"
         "location, and per component the joint basis gives only 2-4 components per Allen area --"
         "too thin to localise. This is full-resolution pixels."
         "\n\nMETHOD. A multinomial logistic decoder (L2, C=0.5, 5-fold grouped by the"
         "scheduler's position blocks) is fitted PER SESSION on the rank-100 temporal component"
         "matrix SVT -- not on LocaNMF -- so that U @ coefficients is a genuine 540 x 640 cortical"
         "map with no component-space intermediary. Coefficients are averaged over folds, the"
         "StandardScaler is undone so they live in feature space, and the four 0.25 s sub-bins are"
         "averaged into one map. Each animal contributes its own epoch mean and those are then"
         "averaged, so an animal with more sessions cannot dominate. Allen CCF boundaries overlaid."
         "Following Musall et al. (Nat Neurosci 2022), with three departures, each MEASURED rather"
         "than assumed:"
         "\n\n(1) SVT RATHER THAN LocaNMF -- theirs, and it costs nothing: SVT features beat"
         "LocaNMF by 0.06-0.09 balanced accuracy, because LocaNMF is a constrained re-description"
         "of the same rank-100 data."
         "\n(2) L2 RATHER THAN THEIR L1. We are making a map, not selecting features, and L1's"
         "choice among correlated predictors is free to change between days. Split-half reliability"
         "of the pre-stroke mean map: L1 0.688, L2 0.715, L2+Haufe 0.960."
         "\n(3) THE HAUFE TRANSFORM, which the paper does not use and which is the single biggest"
         "factor in that table -- single-session reliability goes from 0.32 to 0.85. A decoder"
         "weight vector is a FILTER, and part of its job is cancelling correlated noise, so a"
         "channel carrying NO signal can take a large weight purely as a suppressor. A = Cov(X) x"
         "beta converts it to a PATTERN, cov(channel, decoder output), which is the anatomical"
         "question. READ THE PATTERN FOR WHERE THE SIGNAL IS; the filter answers what the decoder"
         "USES and is a different map -- they correlate at only r = 0.245. A pattern map is NOT"
         "evidence that those pixels are necessary for decoding."
         "\n\nTRIAL BALANCING on the POST-LICK arm only. The `working` class is uniform over"
         "positions by construction (16.1-17.2% each, every animal) because the scheduler presents"
         "positions evenly, so pre-cue and post-cue need none. The `lick` class runs"
         "far-contralateral at 9.2-14.2% against ~19% for near positions, and that skew IS the"
         "deficit, so an unbalanced lick-aligned fit would map how often each spout was attempted"
         "rather than what cortex did."
         "\n\nHOW TO READ IT. The colour scale is PER ROW, so a position is comparable across ITS"
         "OWN epochs -- which is the comparison being made -- and rows are NOT comparable to each"
         "other. The final column is a DIFFERENCE and has its own diverging map and its own limit."
         "Limits are percentiles, not maxima, because one saturated pixel at the mask boundary"
         "would otherwise flatten every real feature."
         "\n\nr IN EACH PANEL IS THE SPLIT-HALF RELIABILITY of that epoch's mean map -- the"
         "ceiling any difference involving it can reach. IT IS NOT A CAVEAT THAT DISCOUNTS THE"
         "RESULT: a split-half correlation of a near-absent signal is low BECAUSE the signal is"
         "near-absent. Far-contralateral acute is r = 0.53 AND 0.47 of its pre-stroke amplitude;"
         "those are one observation, not two. Far-middle falls to 0.48 amplitude while KEEPING"
         "r = 0.86, which is what shows the two are separable."
         "\n\nTHE RESULT, and it converges with the encoder from a completely different"
         "direction. Acute map amplitude relative to each position's own pre-stroke value: near"
         "ipsi 0.67, near middle 1.53, near contra 1.04, far ipsi 1.09, FAR MIDDLE 0.48, FAR"
         "CONTRA 0.47. The two positions that lose more than half their map amplitude acutely are"
         "far-middle and far-contralateral -- position-specific, exactly the pair every other"
         "analysis implicates -- and both recover by subacute (0.83 and 0.91). The encoder's fitted"
         "amplitude factor says the same thing in components rather than pixels: 0.286 acute"
         "against 0.749 pre-stroke, recovering to 0.745 chronic."
         "\n\nCHRONIC RESTS ON TWO ANIMALS AND SIX SESSIONS (PS94 has no chronic epoch; PS92 and"
         "PS93 contribute partially), so read that column with the caution its panel counts state."),
