...
M_COMMON = ("Features = individual LocaNMF component activities (atlas-anchored NMF, r2=0.95, "
            ...
            "ENGAGED = cue trials whose FIRST DETECTED LICK falls within decode.max_rt_s = 2.0 s of the "
...
M_EVOKED = (
    ...
    "\n\nFINDING 1 IS WITHDRAWN AS STATED, and replaced. It read AMPLITUDE RISES IN ALL FOUR "
    "ANIMALS, GRADED BY SEVERITY, from a measure that SUMS |response| across 66 areas -- which "
    "conflates how big a response is with how far it spreads. The common-scale maps separate "
    "them: PS94 PEAK amplitude rises only at close_L (0.039 -> 0.073) and close_center (0.029 "
    "-> 0.055), is flat at close_R, and FALLS at the far positions (far_center 0.018 -> 0.008). "
    "Meanwhile the summed measure rose at far_L (0.278 -> 1.070) with its peak flat at 0.019 -> "
    "0.017. Those reconcile only one way: the response became spatially BROADER, not stronger -- a diffuse bilateral activation replacing a focal one. That also explains the lateralisation collapse (a broad bilateral response has R-L ~ 0 by construction), the crossnobis convergence, and the frozen decoder failing on focal pre-stroke templates. These numbers additionally predate the all-trials and six-position corrections and are being re-derived. "
