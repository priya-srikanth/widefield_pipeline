...
def build_analysis_deck(src: Path, out_path: Path, dates=None, animals=None, tag=None, allow_missing=0,
    ...
    _ROT_LEGEND = (
        ...
        "\n\nACROSS CELLS THE CORRECTION IS A PERMUTATION MAX-STATISTIC, NOT BONFERRONI, and the"
        " reason is structural rather than a preference. The six positions are NOT independent:"
        " the decoder is multinomial, so its class coefficients are identified only up to a"
        " constant shift across classes and the six patterns are coupled by construction -- the"
        " same coupling that makes the mean-referenced family unusable as a deficit measure."
        " Bonferroni assumes independence and is conservative here by an unknown amount. The"
        " max-statistic takes each draw's largest cohort z across the family, and thresholds at"
        " the 95th percentile of those maxima, so the data's own covariance performs the"
        " correction. The family is WINDOW x CONTRAST x POSITION; animals are replicates of one"
        " hypothesis and are deliberately NOT in it."
    ...
    _EPOCH = (
        ...
        ("epoch_14_beta_maps_MEANref_*_*.png",
         ...
         " than annotated, because a green contour is read as a result and a subtitle caveat is"
         " not."
