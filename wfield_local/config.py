...
def load_sessions() -> list[dict]:
    ...
    `fmdir` defaults to None; `behavior_trials` is included only when set."""
    raw = _load("sessions.yaml")["sessions"]
    rows = []
    for animal in raw:
        for date, e in raw[animal].items():
            date = str(date)
            entry = dict(label=f"{animal}_{date}", mc=e["mc"], h5=e["h5"],
                         regime=e.get("regime"), fmdir=e.get("fmdir"))
            if e.get("behavior_trials"):
                entry["behavior_trials"] = e["behavior_trials"]
